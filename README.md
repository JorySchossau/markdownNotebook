...
* `trim:head,N` / `trim:tail,N` - truncate a cell's captured output to its first (`head`) or last (`tail`) `N` lines before it is written back, so large outputs don't bloat the file. If `trim:` is unspecified the default is `trim:head,50`. When lines are actually dropped, a one-line footer (naming the mode, the limit, and how many lines were cut) is appended so you can see that truncation happened. A no-op when the output already fits. Applies only to captured command output; mdnb-authored notices (timeout-kill, `(please wait)`) are not trimmed.
...
- [x] Add a `trim:head,N` and `trim:tail,N` codeblock command to truncate output to the first or last N lines before it is written back. Default to `trim:head,50` so large outputs do not burden the source file or the mdnb server. This default applies when no `trim:` is given. When lines are actually dropped, a one-line footer naming the mode, the limit, and the count cut is appended. A no-op when the output already fits; applies only to captured command output.
