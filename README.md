...
> **When does a cell re-run?** A cell is "dirty" (re-runs on save) when its body changed, **or when any of its commands/arguments changed** — the language id, `source:`/`append:` target, `output:`/`show:` target, `inputs:`, `timeout:`, or `trim:`. Editing a command (e.g. `timeout:5`→`timeout:30`) marks the cell dirty and re-runs it even if the body is byte-identical. The `[s/r/x/k]` state field is excluded — mdnb flips that itself, so it never triggers a rerun on its own.
...
This tier changes the default execution model. Today a runnable cell with no `[ ]` state field **auto-runs whenever it is dirty**. The goal of this tier is the opposite default: **nothing runs unless the user explicitly asks.** mdnb injects a `[s]` (stopped) placeholder into every runnable cell so the notebook is inert by default, and the user drives execution either per-cell or in bulk via new global `:runall` / `:runabove` / `:runbelow` commands. The cell call-dependency graph (`inputs:`/`output:`) and the `[s/r/x/k]` machinery all stay in place — only the *default* flips, and bulk-run commands are layered on top.

> **Behavior change / migration note.** This is a breaking change for existing notebooks: the first save under the new version injects `[s]` into every runnable cell, so previously auto-running notebooks go silent until the user runs them. `-o` (run once) should keep force-running everything so `./mdnb -o file.md` still produces a full build regardless of states — confirm and implement that as one of the items below.

*Design decision to resolve first (flag for the maintainer before coding):* the request says "only run when the user has changed the state to `[r]`". This **conflicts** with the documented existing semantics, where `r` = *running* (set by mdnb itself while a cell is in flight; "don't type this yourself") and `x` = *execute* (the user's force-run trigger). Recommended resolution: keep `x` as the user-typed run trigger (so `r` stays mdnb-internal and `k` stays kill), and treat the request's "`[r]`" as meaning the existing `[x]` execute. The items below assume that resolution; if the maintainer instead wants to repurpose `r` as a user trigger, every "set `[x]`" step becomes "set `[r]`" and the running-state bookkeeping must move elsewhere. Record the chosen mapping in `agents.md` §7/§8.

- [ ] **4.1 Decide and record the run-trigger state.** Resolve the `[r]`-vs-`[x]` conflict above with the maintainer. Whichever char is chosen, the rest of the tier uses it consistently as "the state the user types to make a cell run." Update `agents.md` §7 (grammar) and §8 (execution controls) and the README "Supported Commands" state-field list to match. *(Prerequisite for all other items.)*
- [ ] **4.2 Add an `[s]`-placeholder injection step to the pipeline.** On every save, for each runnable code cell (a cell whose language is a registered runtime — `code:` in frontmatter — or the special `raw` language, i.e. `props.code == true`), if its info string has **no** `[ ]` state field, insert `[s]` immediately after the language id and before any other commands. Example: `` ```sh `` → `` ```sh [s] `` and `` ```python output:simulation.txt `` → `` ```python [s] output:simulation.txt ``. Concretely:
  - [ ] **4.2a** Add a new pipeline proc (e.g. `injectStoppedState` in `mdnb_pipeline.nim`, run after the second `processBodyForCells` in `process`, before `collateSources`) that walks `md.cells`, finds each runnable cell's info-string line (the fence line ending just before `cell.rng.a` — reuse the back-scan already in `writeCellState`), and splices ` [s]` in after the lang id when no `[ ]` field is present.
  - [ ] **4.2b** Make it **idempotent**: never inject into a cell that already has a `[ ]` field, so repeated saves don't double up. (The back-scan for `[` … `]` from `writeCellState` tells you whether a field exists.)
  - [ ] **4.2c** Patch downstream cell byte offsets after each splice via `updatePositionsByOffset` (the injection grows the buffer), exactly like `replaceShortcuts`/`writeIntoCell` do. Re-run is not needed because injection happens before the cells are read for output.
  - [ ] **4.2d** Only inject for runnable cells. A fenced block whose language is **not** in `md.runtimes` (and not `raw`) stays inert — no `[s]`, matching today's "unregistered language = nothing runs" rule.
  - [ ] **4.2e** Write the buffer to disk after injection so the placeholder actually appears in the user's editor (call `md.write`, and ensure the watch loop's self-write-ignoring logic in `main` treats this mtime as mdnb's own so it doesn't feedback-loop).
- [ ] **4.3 Flip the default: dirty cells no longer auto-run.** Change `cellShouldRun` (in `mdnb_run.nim`) so the behavior with no explicit run request is **stop**, not run. After 4.2 every runnable cell carries `[s]`, so the existing `'s'` branch already returns false — but verify the `'\0'` (truly absent) case also does not run (defensive: a cell that somehow has no field must not auto-run under the new model). `markDirtyCells` keeps marking cells dirty exactly as today (the call graph still computes dependencies and propagates dirtiness); a dirty cell simply **waits** in `[s]` until the user runs it. Update the `cellShouldRun` doc comment and the README "When does a cell re-run?" note to describe the new default.
- [ ] **4.4 Keep `[x]`/`[k]` working.** A user typing `[x]` (execute) still force-runs that one cell via the existing path; `[k]` still kills a running cell. No grammar change needed (`stateField` already accepts `[srxk]`). Add a manual test: type `[x]` on a `[s]` cell, save, confirm it runs and settles back to `[s]`.
- [ ] **4.5 Add grammar for the three global commands.** In `mdnb_grammar.nim`, add PEG patterns `runAllPattern`, `runAbovePattern`, `runBelowPattern` modeled on the existing `cleanPattern` (each matches the command token on its own line, e.g. `:runall`, `:runabove`, `:runbelow`, with optional trailing whitespace).
- [ ] **4.6 Detect the commands and record their scope + position.** Extend `replaceShortcuts` (or add a sibling pass in `mdnb_shortcuts.nim`) to, like `:clean`, remove the command line and set flags on `md`. Add three `bool`/position fields to `MarkdownFile` (e.g. `runAll`, `runAboveFrom`, `runBelowFrom`). `:runall` sets `runAll`. `:runabove`/`:runbelow` must capture the **byte offset of the command line** before removal, so "above"/"below" can be mapped to a cell boundary (the nearest cell whose `rng.a` is ≤ / > that offset). Remove the command line and patch offsets as `:clean` does.
- [ ] **4.7 Implement the bulk-run effect.** Add a pipeline step (in `process`, after `markDirtyCells`, before `runCells`) that, when one of the flags from 4.6 is set, selects the target cells in document order and runs them:
  - [ ] **4.7a** `:runall` → every runnable code cell, in document order.
  - [ ] **4.7b** `:runabove` → every runnable code cell whose `rng.a` is at or before the command's original offset, in document order.
  - [ ] **4.7c** `:runbelow` → every runnable code cell whose `rng.a` is at or after the command's original offset, in document order.
  - [ ] **4.7d** For each selected cell, set its state to the run trigger (`[x]` per 4.1) so the existing launch machinery picks it up. Clear the flag(s) after acting so a normal save doesn't re-trigger.
- [ ] **4.8 Run bulk cells **in order**, updating the file after each state change.** The request requires that "after each cell state changes, the file should be appropriately updated" — the cell that changed flips `x`→`r`→`s`, its `output:`/`show:` render, and dependent cells update. The normal path launches cells non-blockingly, which does **not** guarantee order or per-cell file updates. For the bulk commands, run the selected cells **sequentially**: for each, set `[x]`, run `runCells` (which flips to `[r]`, launches, and — for these — should block until the cell finishes so the next one starts only after), reap it (`reapFinished` writes output + flips to `[s]`), then `md.write` so the user sees the progression. Reuse `startRun`/`reapFinished`; the sequencing is a new wrapper. Honor each cell's `timeout:` and the `[k]` kill within this sequential loop. Document that bulk-run is intentionally synchronous (it blocks the watcher for the duration) and why.
- [ ] **4.9 Make `-o` force-run everything regardless of state.** Confirm/extend the `-o` path so a one-shot run executes every runnable cell even if it is `[s]` (treat `-o` like `:runall` for all watched files). This preserves the "clean build" contract of `-o` and gives users a migration path off the old auto-run default.
- [ ] **4.10 Document the new model end-to-end.**
  - [ ] **4.10a** Update README "Supported Commands": rewrite the `[s/r/x/k]` state-field description to state that runnable cells default to `[s]` (stopped, inert) and that mdnb injects the placeholder; clarify the run trigger per 4.1; add `:runall` / `:runabove` / `:runbelow` to the Non-Code Fence Commands list alongside `:clean`, each with a one-line description of scope.
  - [ ] **4.10b** Add a before/after example to README mirroring the one in the request (`` ```sh `` → `` ```sh [s] ``, and the `output:simulation.txt` variant).
  - [ ] **4.10c** Update the "How does it work?" steps list and the "When does a cell re-run?" callout to reflect stopped-by-default.
  - [ ] **4.10d** Sync `agents.md`: §4 pipeline diagram (add the injection + bulk-run steps), §6/§7 grammar + command list, §8 "I want to add per-cell execution controls" / "I want to change when cells re-run", and §12 roadmap (mark this tier done as items land). The contributing guide (§10.5) requires README and agents.md stay in sync.
- [ ] **4.11 Manual regression pass.** Verify against the §2 smoke demo plus a multi-cell, multi-language file (the README worked example): (a) first save injects `[s]` everywhere and nothing runs; (b) `[x]` on one cell runs just it; (c) `:runall` runs all in order with visible `x`→`r`→`s` progression and outputs landing; (d) `:runabove`/`:runbelow` run the correct halves; (e) `inputs:`/`output:` dependency cells still get marked dirty (and run when their bulk scope includes them); (f) `-o` still produces a full build; (g) `[k]` still kills a running cell mid-bulk-run.
