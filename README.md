...
* `header:filename` - declare `filename` as an input this cell consumes. The cell becomes a *dependency* of any cell that `output:`s `filename`: when that producing cell changes, this cell is rerun too, even if its own source is unchanged. Dirtiness propagates transitively, so chains of `header:`→`output:` relationships all rebuild. (`header:` files are never written by mdnb — they are pure inputs.)
...
- [x] Build a cell call-dependency graph. A cell is considered dirty (and rerun) when a cell whose output it consumes changes, not only when its own source or output file is missing. A cell declares what it consumes via `header:<file>`; it depends on any cell that `output:`s that same file, and dirtiness propagates transitively through such chains. (`header:` is the dependency-input signal because mdnb never writes a cell's header file — it's a pure input — whereas a cell's own `source:`/`output:` are files it *writes*.) Changing cell A invalidates cells that depend on A's output.
