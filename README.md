...
Optional flags:
* `-o` — run once and exit (also forces a clean build) instead of watching. A clean build **force-runs every runnable code cell regardless of its `[s/r/x/k]` state**, so `./mdnb -o file.md` always produces a full notebook — the stopped-by-default model does not apply here.
...
- [x] **4.7/4.8 Bulk-run effect, in order.** `runBulk` (in `mdnb_pipeline.nim`) dispatches on `runMode` and selects the target cells in document order (`:runall` = all; `:runabove` = indices < boundary; `:runbelow` = indices ≥ boundary). `runCellsSequential` (in `mdnb_run.nim`) runs the selection one cell at a time, fully completing each (`x`→`r`→`s`, output written, `show:` cell filled) before the next starts — giving the visible per-cell progression the request requires. Each cell is forced to `[x]`, launched via `startRun`, then synchronously reaped via `reapThis` (which drains the pipe each spin, honors `timeout:` and `[k]`, and calls `reapFinished`). This deliberately **blocks the watcher** for the duration; ordering and per-cell file updates are the point and aren't possible with the non-blocking path. Dependencies still apply. Side fix: `startRun`/`reapFinished`/`reapTimeout` now keep the in-memory cell state synced with disk (`x`→`r`, `r`→`s`) — the watch loop re-reads the file each cycle so it never noticed, but the sequential path reuses one `md` across cells and needs the in-memory state to track disk.
- [x] **4.9 `-o` force-runs everything.** The run-once path sets a `forceRunAll` flag (in `mdnb_run.nim`) for its one `process()` pass, so `cellShouldRun` returns true for every code cell regardless of `[s]` — preserving `-o`'s clean-build, full-output contract under stopped-by-default.
